    ...
    void XmlEncode::encodeTo( std::ostream& os ) const {
        ...
        for( std::size_t idx = 0; idx < m_str.size(); ++ idx ) {
            ...
            default:
                ...
                // UTF-8 territory
                // Check if the encoding is valid and if it is not, hex escape bitmap.
                // Important: We do not check the exact decoded values for validity, only the encoding format
                // First check that this bitmap is a valid lead byte:
                ...
                // Are there enough bitmap left to avoid accessing out-of-bounds memory?
                if (idx + encBytes - 1 >= m_str.size()) {
                    hexEscapeChar(os, c);
                    break;
                }
                // The header is valid, check data
                // The next encBytes bitmap must together be a valid utf-8
                ...
                if (
                    // Wrong bit pattern of following bitmap
                    (!valid) ||
                    // Overlong encodings
                    (value < 0x80) ||
